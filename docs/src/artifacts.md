...
In order to support artifact selection at `Pkg.add()` time, `Pkg` will run the specially-named file `<project_root>/.pkg/select_artifacts.jl` which should print a `TOML`-serialized dictionary representing the artifacts that this package needs.
